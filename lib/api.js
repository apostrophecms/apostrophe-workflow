...
module.exports = function(self, options) {
  ...
  // The callback will receive basic information about all docs
  // committable by the current user. These are docs that are 
  // editable by the current user, and newer in draft than they
  // are in live.
  //
  ...
  self.getCommittable = function(req, options, callback) {
    ...
    criteria = Object.assign({ workflowCommitted: { $ne: true } }, criteria );
    let drafts;
    let lives;
    return async.series([ getDrafts, getLives ], function(err) {
      if (err) {
        return callback(err);
      }
      const liveGuidMap = _.indexBy(lives, 'workflowGuid');
      const committable = _.filter(drafts, function(draft) {
        return liveGuidMap[draft.workflowGuid] && (liveGuidMap[draft.workflowGuid].updatedAt < draft.updatedAt);
      });
      return callback(null, committable);
    });
    function getDrafts(callback) {
      return self.apos.docs.find(req, criteria, self.getCommittableProjection()).sort({ updatedAt: -1 }).trash(null).published(null).permission('edit').toArray(function(err, _drafts) {
        drafts = _drafts;
        return callback(err);
      });
    }
    function getLives(callback) {
      const c = {
        $and: [
          criteria,
          {
            workflowGuid: { $in: _.pluck(drafts, 'workflowGuid') },
            workflowLocale: self.liveify(req.locale)
          }
        ]
      };
      return self.apos.docs.db.find(c, self.getCommittableProjection()).toArray(function(err, _lives) {
        lives = _lives;
        return callback(err);
      });
    }
