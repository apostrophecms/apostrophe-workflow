...
module.exports = function(self, options) {
  ...
  self.revert = function(req, commitId, callback) {
    ...
    return async.waterfall([getDocAndCommit, copyIncludedProperties, update], callback);

    function getDocAndCommit(cb) {
      return self.findDocAndCommit(req, id, cb);
    }

    function copyIncludedProperties(doc, commit, cb) {
      return self.copyIncludedProperties(req, commit.from, doc, () => {
        cb(null, doc);
      });
    }

    function update(draft, cb) {
      return self.apos.docs.update(req, draft, cb);
    }
  };
