...
module.exports = function(self, options) {
  ...
  self.getAvailability = function(req, workflowGuid, locale, callback) {
    return self.apos.docs.find(req, { workflowGuid: workflowGuid, workflowLocale: self.draftify(locale) }).trash(null).published(null).workflowLocale(locale).toObject(function(err, doc) {
      if (err) {
        return callback(err);
      }
      if (!doc) {
      ...
      if ((doc.updatedAt.getTime() - doc.createdAt.getTime()) < 5000) {
        return callback(null, 'newInTrash');
      }
      return callback(null, 'inTrash');
    });
