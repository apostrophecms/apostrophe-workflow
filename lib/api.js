...
module.exports = function(self, options) {
  ...
  // committable by the current user in the current locale. These are docs that are 
  ...
  self.getCommittable = function(req, options, callback) {
    ...
    return self.findDocs(req, criteria, req.locale).projection(self.getCommittableProjection()).sort({ updatedAt: -1 }).permission('edit').toArray(function(err, docs) {
      if (err) {
        return callback(err);
      }
      // Ignore docs not subject to workflow
      docs = _.filter(docs, function(doc) {
        return self.includeType(doc.type);
      });
      docs = self.filterCommittableDrafts(req, docs);
