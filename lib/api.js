...
module.exports = function(self, options) {
  ...
  // The callback will receive information about docs
  // committable by the current user in the current locale.
  // These are docs that are editable by the current user,
  // committable by the current user, and modified in draft.
  //
  ...
  //
  // To avoid exhausting memory, the response is always paginated.
  // Pagination is available via `options.page` and `options.perPage`.
  // Page numbers begin with 1. By default you will receive the results
  // for page 1, with 10 results per page.
  ...
  self.getCommittable = function(req, options, callback) {
    ...
    const skip = Math.max((options.page || 1) - 1, 0) * perPage;
    const limit = perPage;
    ...
    const types = Object.keys(self.apos.docs.managers).filter(function(type) {
      ...
      return (type !== 'trash') && self.includeType(type) && self.apos.permissions.can(Object.assign({}, req, { workflowLocale: self.liveify(req.locale) }), 'edit-' + type);
    });
    if (!types.length) {
      return callback(null, { pages: 0, results: [] });  
    }
    const query = self.findDocs(req, options.criteria || {}, req.locale).projection(self.getCommittableProjection()).and({
      workflowLocale: { $exists: 1 },
      type: { $in: types },
      workflowModified: true
    }).sort({ updatedAt: -1 }).permission('edit');
    var count, results;
    return async.series([
      count,
      fetch
    ...
    function count(callback) {
      return query.toCount(function(err, _count) {
        count = count;
        return callback(err);
      });
    }
    function fetch(callback) {
      return query.toArray(function(err, results) {
        results = results;
        return callback(err);
      });
    }
  ...
  self.getEditable = function(req, ids, options, callback) {
    ...
    function getCommittable(callback) {
      related = _.filter(related, { _edit: true });
      ...
      var all = relatedModified.concat(relatedUnmodified);
      committable = self.filterCommittableDrafts(req, all);
      return callback(null);
    }
  };
