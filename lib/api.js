...
module.exports = function(self, options) {
  ...
  // Find the specified doc by id, even if it is in the trash or unpublished,
  // without regard to locale (except for permissions checks). Areas and joins
  // are not loaded.
  
  self.findDocForEditing = function(req, docId, callback) {
    // The implementation does not call permission('edit') because
    // that code is very conservative and cannot figure out what docs
    // a user without the generic "edit" permission can edit in the
    // generic case with no type filter. That needs a look in Apostrophe core.
    // But we can address it by checking for the `_edit` annotation instead.
    return self.apos.docs.find(req, { _id: docId }).trash(null).published(null).workflowLocale(null).areas(false).joins(false).toObject(function(err, doc) {
      if (err) {
        return callback(err);
      }
      if (!doc) {
  ...
  // except that any locales that are not committable by the
  // user indicated by `req` are given the `disabled: true` flag.
  // Useful when rendering the locale tree for purposes of picking
  // locales to commit to. Note that committable locales may be
  // nested beneath disabled locales.

  self.getCommittableNestedLocales = function(req, doc) {

    var nestedLocales = _.cloneDeep(self.nestedLocales);
    _.each(self.locales, function(locale, name) {
      var _req = _.clone(req);
      _req.locale = name;
      var can = self.apos.permissions.can(_req, 'edit-' + doc.type);
      if ((!can) || can.draftOnly) {
        disable(name);
      }
    });    
