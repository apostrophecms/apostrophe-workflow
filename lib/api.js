...
module.exports = function(self, options) {
      ...
      return async.eachSeries(joins, function(join, callback) {
        ...
        function findWorkflowGuids(callback) {
          if (join.field.type === 'joinByOne') {
            return self.apos.docs.db.findWithProjection({ _id: { $in: [ join.doc[join.field.idField] ] } }, { workflowGuid: 1 }).toArray(function(err, docs) {
              if (err) {
                return callback(err);
              }
            ...
            return self.apos.docs.db.findWithProjection({ _id: { $in: join.doc[join.field.idsField] || [] } }, { workflowGuid: 1 }).toArray(function(err, docs) {
        ...
        function findSecondLocaleIds(callback) {
          return self.apos.docs.db.findWithProjection({ workflowGuid: { $in: workflowGuids }, workflowLocale: toLocale }, { _id: 1, workflowGuid: 1 }).toArray(function(err, docs) {
            if (err) {
              return callback(err);
            }
  ...
  self.commit = function(req, from, to, callback) {
    ...
    function markUnmodified(callback) {
      return self.apos.docs.db.update({
        _id: from._id
      }, {
        $set: {
  ...
  // The callback will receive information about ALL docs
  // committable by the current user. These are docs that are 
  // editable by the current user, and modified in draft.
  //
  // If `options.criteria` is present it is merged with the MongoDB
  // criteria. You may use this to restrict the response.
  //
  // The response is specific to the current locale.
  //
  // The response is sorted to put most recently modified docs first.

  self.getCommittable = function(req, options, callback) {

    let criteria = options.criteria || {};
    criteria = Object.assign({ workflowModified: true, type: { $ne: 'trash' } }, criteria);
    return self.apos.docs.find(req, criteria, self.getCommittableProjection()).sort({ updatedAt: -1 }).trash(null).published(null).joins(false).areas(false).permission('edit').toArray(callback);
  };
  ...
  // Returns the projection to be used when fetching all
  // committable docs. Should be enough to generate permalinks
  // and display last editor information.
  
  self.getCommittableProjection = function() {
    return {
      title: 1,
      slug: 1,
      path: 1,
  ...
  self.getEditable = function(req, ids, options, callback) {
    ...
    console.log(req.url + ': ENTRY for ', ids);
    var related = [];

    // TODO you don't need this cache anymore, but you
    // need to know which are committable (editable in live).
    // Use permissions knowledge to figure that out faster
    ...
    return async.series([
      ...
      _.each(relatedModified.concat(relatedUnmodified), function(doc) {
        if (_.has(cache.liveVersions, doc._id)) {
          if (cache.liveVersions[doc._id]._edit) {
            committable.push(doc);
          }
        }
      });
      console.log(relatedModified.length, relatedUnmodified.length, committable.length);
    ...
    function getKnown(callback) {
      ...
      return async.eachSeries(ids, function(id, callback) {
        return self.getDraftAndLive(req, id, { permission: false }, function(err, draft, live) {
          ...
          related.push(draft);
          cache.draft[draft.workflowGuid] = draft;
          cache.liveVersions[draft.workflowGuid] = live;
          return callback(null);
        });
      }, callback);
    }
    ...
    function diff(callback) {
      ...
      return self.getModified(req, ids, function(err, result) {
        if (err) {
          return callback(err);
        }
        relatedModified = result.modified;
        relatedUnmodified = result.unmodified;
        return callback(null);
      });
    }
  ...
  // Takes draft ids. Callback is invoked with `(null, result)` where
  // `result` is an object with `modified` and `unmodified` properties.
  // `modified` contains draft documents whose content has been modified
  // relative to the live version, while `unmodified` contains those
  // whose content has not.

  self.getModified = function(req, ids, callback) {
    ...
    return self.apos.docs.db.find({
      ...
      return callback(null, {
        modified: _.filter(docs, function(doc) {
          return doc.workflowModified;
        }),
        unmodified: _.filter(docs, function(doc) {
          return !doc.workflowModified;
        })
      });
    });
  ...
  self.isModified = function(req, draft, callback) {
    ...
    return self.apos.docs.db.findOne({
      ...
      workflowLocale: self.liveify(draft._id)
    }, function(err, live) {
      if (err) {
        return callback(err);
      }
      ...
      return self.draftAndLiveAreEqual(req, draft, live, function(err, equal) {
        return callback(err, !equal);
      });
    });
  ...
  self.draftAndLiveAreEqual = function(req, draft, live, callback) {
    ...
    return self.resolveRelationships(req, _draft, live.workflowLocale, function(err) {
      ...
      return callback(null, _.isEqual(_draft, _live));
    });
  };
  ...
  self.revertToLive = function(req, id, callback) {
    ...
    function applyPatch(callback) {
      ...
      return async.series([ getDraft, resolveToDestination, applyPatch, update, move, committed ], callback);
