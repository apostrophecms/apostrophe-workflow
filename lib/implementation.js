...
module.exports = function(self, options) {
  ...
  self.composeOptions = function() {
    ...
    if (self.options.prefixes === true) {
      ...
      _.each(self.locales, function(locale, name) {
        ...
        self.prefixes[name] = self.liveify(name);
      });
  ...
  self.ensurePageSlugPrefix = function(doc) {
    if (!(self.prefixes && self.prefixes[self.liveify(doc.workflowLocale)] && self.apos.pages.isPage(doc) && doc.workflowLocale)) {
      return;
    }
    var prefix, matches;
    ...
    if (!matches) {
      ...
      existing = matches[1];
      if (('/' + existing) === self.prefixes[doc.workflowLocale]) {
        // Good to go
      } else {
