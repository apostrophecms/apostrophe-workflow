...
module.exports = function(self, options) {
  ...
  // drafts or missing live docs. Setting this option to `related` works similarly
  // but fully replicates all docs related by joins and widgets to the foundational docs.
  ...
  self.addMissingLocales = function(req, options, callback) {
    if (self.options.replicateAcrossLocales !== true) {
      ...
      let guids;
      return Promise.try(function() {
        return self.apos.docs.db.find({ workflowLocale: 'es-mx-draft' }).toArray();
      }).then(function(docs) {
        return self.discoverAlwaysReplicated();
      }).then(function(workflowGuids) {
        guids = workflowGuids;
        return self.apos.docs.db.find({ workflowGuid: { $in: workflowGuids }, workflowLocale: 'es-mx-draft' }).toArray();
      }).then(function(docs) {
        return guids;
      }).then(function(workflowGuids) {
