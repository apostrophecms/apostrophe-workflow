...
module.exports = function(self, options) {
  ...
  self.composeLocales = function() {
    ...
    _.each(_.keys(self.locales), function(locale) {
      if (locale.match(/-draft$/)) {
        ...
        self.locales[locale].depth = self.locales[self.liveify(locale)].depth;
      }
    });
  };
  ...
  self.getCreateSingletonOptions = function(req) {
    ...
    return {
      ...
      committersSeeSubmit: self.options.committersSeeSubmit
    };
  };
  ...
  self.addMissingLocales = function(req, options, callback) {
    if (self.options.replicateAcrossLocales === false) {
      ...
      // 1. Deal with the parked pages and global doc via the usual complex algorithm (factored out).
      // 3. Deal with everything else, but we are only interested in making sure drafts have live counterparts
      // and vice versa.
      return Promise.try(function() {
        return Promise.promisify(self.replicateAllMissingLocales)(req, Object.assign({
          criteria: {
            $or: [
              { parkedId: { $exists: 1 } },
      ...
      return self.replicateAllMissingLocales(req, options, callback);
    }
  };

  // Fill in fundamental-related and, if necessary, fundamental docs that do
  // not exist yet in the given locales, by using the versions in the immediate
  // parent locales.
  //
  // This method is invoked by the `add-missing-locales` task if the
  // --replicate-related-documents=localea,localeb,... option is present.
  //
  // Used for brand new locales, even if `replicateAcrossLocales: false` is
  // present. This ensures that not just parked documents and the global doc,
  // but the docs that are closely related to them and make them useful, exist
  // in the new locale as well so it can be more effectively developed.
  //
  // If both parent and child locales appear in `locales`, parents are
  // processed first to prevent chicken and egg problems.

  self.replicateRelatedDocuments = function(locales) {
    locales.sort(function(a, b) {
      var diff = self.locales[a].depth - self.locales[b].depth;
      if (diff < 0) {
        return -1;
      } else if (diff === 0) {
        return 0;
    ...
    locales = locales.map(self.draftify);
    locales = _.uniq(locales);
    const req = self.apos.tasks.getReq();
    return Promise.mapSeries(locales, processLocale);

    function processLocale(locale) {
      if (!getParent(locale)) {
        // This algorithm has no bearing on the root locale
        return;
      }
      self.apos.utils.info('Replicating related docs to new locale ' + locale + '...');
      let total;
      ...
      let guids;
      return self.discoverRelatedAndFundamental(getParent(locale)).then(function(_guids) {
        guids = _guids;
        total = guids.length;
        if (!guids.length) {
        ...
        return Promise.try(function() {
          ...
          return Promise.promisify(self.resolveDeferredRelationships)();
        });
        ...
        function processGroup(subset, runOptions) {
          ...
          if (runOptions.batchSize && runOptions.guids) {
            range.workflowGuid.$in = runOptions.guids.slice(runOptions.nextIndex || 0, (runOptions.nextIndex || 0) + runOptions.batchSize);
          };
          ...
          return Promise.try(function() {
            ...
            return Object.keys(byGuid).filter(function(guid) {
              const missing = 2 - byGuid[guid].length;
              return missing > 0;
            });
            ...
            const parentLocale = getParent(locale);
            relevantGuids.sort(function(a, b) {
              ...
              function getDoc(guid) {
                return byGuid[guid].find(function(doc) {
                  return doc.workflowLocale === parentLocale;
                });
              }
            });

            // map with concurrency 1 is NOT equivalent to mapSeries, because map
            // does not guarantee order of execution, even with concurrency 1.
            // Order of execution is important for page tree operations
            const mapFunction = (runOptions.concurrent === false)
              ? function(items, iterator) {
                return Promise.mapSeries(items, iterator);
              } : function(items, iterator) {
                return Promise.map(items, iterator, { concurrency: 10 });
              }
            ;

            return mapFunction(relevantGuids, function(guid) {
              return self.apos.docs.db.findOne({
                workflowGuid: guid,
                workflowLocale: parentLocale
              }).then(function(sourceDoc) {
                if (!sourceDoc) {
                  self.apos.utils.warn('Doc disappeared during replication, leaving unreplicated on this pass');
                  return;
                }
                if (sourceDoc.slug.match(/^\//)) {
                }
                const afterSaveOptions = Object.assign({}, options, {
                  permissions: false,
                  workflowMissingLocalesLive: true,
                  workflowMissingLocalesLocales: [ locale, self.liveify(locale) ],
                  workflowResolveDeferred: true
                });
            ...
            if (runOptions.batchSize) {
              ...
              if (nextIndex >= runOptions.guids.length) {
                return;
              }
              return processGroup(subset, Object.assign({}, runOptions, {
                nextIndex: nextIndex
    ...
    function getParent(locale) {
      return _.findLast(self.locales[locale].ancestors);
    }
  };

  // Identify the documents that must always be replicated,
  // even though replicateAcrossLocales is `false`.
  // These are the parked pages and the global doc. Returns
  ...
  self.discoverRelatedAndFundamental = function(locale) {
    let fundamental;
    return Promise.try(function() {
      return self.discoverAlwaysReplicated();
    }).then(function(guids) {
      fundamental = guids;
      return self.discoverRelated(locale, fundamental);
    }).then(function(guids) {
      return fundamental.concat(guids);
    });
  };
  ...
  self.discoverRelated = function(locale, workflowGuids) {
    let seen = {};
    if (!workflowGuids.length) {
      return workflowGuids;
    }
    return Promise.try(function() {
      return self.apos.docs.db.find({ workflowGuid: { $in: workflowGuids }, workflowLocale: locale, trash: { $ne: true } }).toArray();
  ...
  self.updatePerConfiguration = function(callback) {
    ...
    function fixBody(oldFact, newFact, callback) {
      ...
      function addMissingLocales(callback) {
        const replicateToLocales = _.difference(newFact.locales, (oldFact && oldFact.locales) || []);
        const argv = Object.assign(self.apos.argv, { 'replicate-related-documents': replicateToLocales.join(',') });
        return self.addMissingLocalesTask(self.apos, argv, callback);
      }
