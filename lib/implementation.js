...
module.exports = function(self, options) {
  ...
  self.addMissingLocales = function(req, options, callback) {
    ...
    return Promise.try(function() {
      ...
      function run(subset, runOptions) {
        // function getTotal() {
        //   if (total === undefined) {
        //     return Promise.delay(100).then(getTotal);
        //   }
        //   return total;
        // }
        console.log('running for ', subset, runOptions);
        let lastGuid;
        const clauses = runOptions.lastGuid ? [
          {
            workflowGuid: { $gt: runOptions.lastGuid }
          }
        ] : [];
        ...
        if (options.batchSize) {
          cursor.limit(options.batchSize);
        }
        return Promise.try(function() {
          ...
          lastGuid = _.last(docs).workflowGuid;
          console.log('AHA ' + lastGuid);
          ...
          return Promise.mapSeries(locales, function(locale) {
            ...
            return Promise.mapSeries(relevantGuids, function(guid) {
              return self.apos.docs.db.findOne({
                workflowGuid: guid,
                workflowLocale: getSourceLocale(guid)
          ...
          if (lastGuid && runOptions.batchSize) {
            return run(subset, Object.assign({
              lastGuid: lastGuid
            }, runOptions));
          }
        });
