...
module.exports = function(self, options) {
              ...
              function replaceIdsRecursively(doc) {
                _.each(doc, function(val, key) {
                  ...
                  if ((typeof (val) === 'string') && (val.length < 100)) {
                    if (idsToNew[val]) {
                      console.warn('Correcting ' + doc[key] + ' to ' + idsToNew[val] + ' in join');
  ...
  // Reconfigure the i18n npm module to work with
  // our workflow locales
  self.reconfigureI18n = function() {
    // includes draft locales
    var locales = _.keys(self.locales);
    // make sure the draft locale dictionaries are just fresh copies
    // of the live locale dictionaries. Live/draft is not a meaningful
    // distinction for static content
    var directory = self.apos.modules['apostrophe-i18n'].directory || (self.apos.rootDir + '/locales');
    _.each(locales, function(locale) {
      if (locale.match(/-draft$/)) {
        console.log('copying');
        var live = self.liveify(locale);
        fs.writeFileSync(directory + '/' + locale + '.json', 
          fs.readFileSync(directory + '/' + live + '.json')
        );
      }
    });
    var i18nOptions = self.apos.modules['apostrophe-i18n'] || {};
