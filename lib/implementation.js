  ...
  self.addMissingLocales = function(req, options, callback) {
    return Promise.try(function() {
      ...
      docs.forEach(function(doc) {
        byGuid[docs.workflowGuid] = byGuid[docs.workflowGuid] || [];
        byGuid[docs.workflowGuid].push(doc);
      });
      return Object.keys(byGuid).filter(function(guid) {
        return byGuid[guid].length < self.locales.length;
      });
    }).then(function(inadequateGuids) {
      return Promise.mapSeries(Object.keys(self.locales), function(locale) {
        ...
        return Promise.mapSeries(relevantGuids, function(guid) {
          const missingLocales = Object.keys(self.locales).find(function(locale) {
            return !byGuid[guid].includes(locale);
          });
          ...
          const sourceLocale = bestAvailableAncestor || byGuid[guid][0].workflowLocale;
          return self.apos.docs.db.findOne({
            workflowGuid: guid,
            workflowLocale: sourceLocale
          }).then(function(sourceDoc) {
            if (!sourceDoc) {
              self.apos.utils.warn('Doc disappeared during replication, leaving unreplicated on this pass');
              return;
            }
            const afterSaveOptions = _.assign({}, options, {
              permissions: false,
