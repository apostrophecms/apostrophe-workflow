...
module.exports = function(self, options) {
  ...
  // 2. Sort by depth, deepest in tree first.
  // 3. Take the deepest one. Identify its closest ancestor in the tree which is not also
  // on the "missing list" (if it has no such ancestor, use the locale with the most docs as fallback).
  // Run through docAfterSave with a restriction to descendants of that ancestor.
  // 4. Remove all other descendants of that same parent from the actionable list.
  // 5. Continue with the next locale on the list.
  // 6. As a fallback to catch any straggler docs (see the 90% rule above), repeat this process
  // for the first root locale, unless it was already run.
  //
