    ...
    middleware: function(req, res, next) {
      ...
      if (req.query.workflowLocale) {
        ...
        if (self.options.hostnames) {
          ...
          hostname = matches[2];
          if (hostname) {
            to = to.replace('//' + hostname, '//' + self.hostnames[locale]);
          } else {
            // Well that shouldn't be possible, but add the default hostname
            to = to.replace('//', '//' + self.hostnames[self.defaultLocale]);
          }
