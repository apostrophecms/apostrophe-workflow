    ...
    middleware: function(req, res, next) {
      ...
      if (req.headers && req.headers['apostrophe-locale']) {
        ...
        if (_.has(self.locales, req.headers['apostrophe-locale'])) {
          ...
          if (self.apos.hasOwnProperty('i18n')) {
            self.apos.i18n.setLocale(req, req.locale);
          }
          console.log('locale is now ' + req.locale);
          return next();
        }
      } else {
        if (req.url.match(/module/)) {
          console.log('header not present on ' + req.url);
        }
      }
