apos.define('apostrophe-permissions-schema-field', {
  ...
  construct: function(self, options) {
    ...
    var superRefreshAdmin = self.refreshAdmin;
    self.refreshAdmin = function($fieldset, initial) {
      superRefreshAdmin($fieldset, initial);
      var $admin = $fieldset.find('input[name="admin"]');
      if ($admin.prop('checked')) {
        $fieldset.find('input[type="checkbox"],select').each(function() {
          return update($(this), true);
        });
      } else if (!initial) {
        $fieldset.find('input[type="checkbox"],select').each(function() {
          return update($(this), false);
        });
      }
      function update($el, admin) {
        $el.val(admin ? 'commit' : '');
      }
    }; 
  }
