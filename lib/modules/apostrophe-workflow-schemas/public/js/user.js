...
apos.define('apostrophe-schemas', {
  ...
  construct: function(self, options) {
      ...
      return apos.modules['apostrophe-workflow'].api('get-live',
        ...
        function(data) {
          ...
          return async.eachSeries(schema, function(field, callback) {
            ...
            $draft.find('label:first').prepend($draftControls);
            ...
            $live.find('label:first').prepend($liveControls);
    ...
    self.enableSlug = function($title, $slug, title, slug) {
      ...
      if ((!workflow.options.prefixes) || (!workflow.options.prefixes[workflow.liveLocale]) || (!slug.page)) {
      ...
      var prefix = workflow.options.prefixes[liveLocale];
      $title.on('change keyup paste', function(e) {
        ...
        if (!matches) {
          ...
          if (('/' + existing) === workflow.options.prefixes[liveLocale]) {
            ...
            // There is no existing prefix matching the locale
            slug = prefix + slug;
          }
        }
        if (slug !== slugWas) {
    ...
    self.addWorkflowPermissionsFieldType = function() {
      apos.schemas.addFieldType({
        name: 'apostrophe-workflow-permissions',
        populate: self.workflowPermissionsPopulate,
        convert: self.workflowPermissionsConvert
      });
    };

    self.findWorkflowPermissionsCheckbox = function($fieldset, name, val) {
      return $fieldset.find('input[name="' + name + '"][value="' + val + '"]');
    };

    self.findWorkflowPermissionsLocaleSelect = function($fieldset, name, val, locale) {
      return $fieldset.find('select[name="' + name + 'Locales[' + val + '][' + locale + ']"]');
    };

    self.workflowPermissionsPopulate = function(object, name, $field, $el, field, callback) {
      var $fieldset = apos.schemas.findFieldset($el, name);
      _.each(object[name] || [], function(val) {
        self.findWorkflowPermissionsCheckbox($fieldset, name, val).prop('checked', true);
      });
      _.each(object[name + 'Locales'] || {}, function(locales, permission) {
        _.each(locales, function(locale, localeName) {
          var $select = self.findWorkflowPermissionsLocaleSelect($fieldset, name, permission, localeName.replace(/-draft$/, ''));
          var isDraft = localeName.match(/-draft/);
          if (isDraft && ($select.val() !== 'commit')) {
            $select.val('edit');
          } else if (!isDraft) {
            $select.val('commit');
          }
    ...
    self.workflowPermissionsConvert = function(data, name, $field, $el, field, callback) {
      var $fieldset = apos.schemas.findFieldset($el, name);
      data[name] = [];
      data[name + 'Locales'] = {};
      _.each(field.choices, function(choice) {
        if (self.findWorkflowPermissionsCheckbox($fieldset, name, choice.value).prop('checked')) {
          ...
          _.each(field.locales, function(locale, localeName) {
            if (localeName.match(/-draft$/)) {
              return;
            }
            var $select = self.findWorkflowPermissionsLocaleSelect($fieldset, name, choice.value, localeName);
            var val = $select.val();
            if (val === 'edit') {
              data[name + 'Locales'][choice.value][localeName + '-draft'] = true;
            } else if (val === 'commit') {
              data[name + 'Locales'][choice.value][localeName + '-draft'] = true;
