      ...
      function parkBody(callback) {
        ...
        return async.eachSeries(locales, function(locale, callback) {
          ...
          return async.eachSeries(parked, function(item, callback) {
            convertToLocalizedSlug(item);
            return self.implementParkOne(req, item, callback);
          }, callback);

          function convertToLocalizedSlug (item) {
            var locale = self.apos.modules['apostrophe-workflow'].liveify(req.locale);

            if (typeof item.slug === 'object') {
              if(!item.slug[locale] && !item.slug._default) {
                throw new Error('apostrophe-workflow: mising default value in localized parked page');
              }
              item.slug = item.slug[locale] || item.slug._default;
            }

            if (typeof item._children) {
              _.each(item._children, function(child) {
                convertToLocalizedSlug(child);
              });
            }
          }
