...
apos.define('apostrophe-pages-editor', {
  construct: function(self, options) {
    ...
    self.afterSave = function(callback) {
      if (self.submitting) {
        return apos.modules['apostrophe-workflow'].submit([ self.savedPage._id ], superAfterSave);
      }
      if (self.committing) {
        return apos.modules['apostrophe-workflow'].commit([ self.savedPage._id ], superAfterSave);
      }
      if (self.forceExporting) {
        return apos.modules['apostrophe-workflow'].forceExport(self.savedPage._id, superAfterSave);
      }
      if (self.accessingHistory) {
        return apos.modules['apostrophe-workflow'].history(self.page._id, superAfterSave);
      }
