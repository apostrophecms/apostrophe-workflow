    ...
    self.beforeShow = function(callback) {
      ...
      self.link('apos-workflow-commit', function() {
        ...
        return self.workflowSaveThen(function(callback) {
          ...
          return workflow.getEditable({ ids: [ self.savedPiece._id ], related: true }, function(err, result) {
            apos.ui.globalBusy(false);
            if (!err) {
              workflow.commit(_.intersection(result.committable, result.modified), { leadId: self.savedPiece._id }, callback);
            }
          });
        });
      });
      ...
      self.link('apos-workflow-force-export-related', function() {
        return self.workflowSaveThen(function(callback) {
          return apos.modules['apostrophe-workflow'].forceExportRelated(self.savedPiece._id, callback);
        });
      });
