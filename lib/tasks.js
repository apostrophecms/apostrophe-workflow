...
module.exports = function(self, options) {
  ...
  self.addMissingLocalesTask = function(apos, argv, callback) {
    ...
    function missingSomeLocalesDraft(callback) {
      return self.apos.migrations.eachDoc({ workflowLocale: self.draftify(self.defaultLocale) }, 5, function(doc, callback) {
        if (!self.includeType(doc.type)) {
          return setImmediate(callback);
        }
        doc.workflowResolveDeferred = true;
        return self.docAfterSave(req, doc, { permissions: false, workflowMissingLocalesSubset: 'draft', workflowMissingLocalesLive: argv.live ? true : 'liveOnly' }, function(err) {
          return callback(err);
        });
      }, callback);
    }
    ...
    function missingSomeLocalesLive(callback) {
      return self.apos.migrations.eachDoc({ workflowLocale: self.defaultLocale }, 5, function(doc, callback) {
        ...
        doc.workflowResolveDeferred = true;
        return self.docAfterSave(req, doc, { permissions: false, workflowMissingLocalesSubset: 'live', workflowMissingLocalesLive: argv.live ? true : false }, function(err) {
          return callback(err);
        });
      }, callback);
    }
