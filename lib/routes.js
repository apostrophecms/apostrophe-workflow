...
module.exports = function(self, options) {
  ...
  self.route('post', 'committable-modal', function(req, res) {
    if (!req.user) {
      // Confusion to the enemy
      return res.status(404).send('not found');
    }
    return res.send(self.render(req, 'committable-modal.html'));
  });
  
  self.route('post', 'committable-list', function(req, res) {
    var page = self.apos.launder.integer(req.body.page, 1);
    return self.getCommittable(req, { page: page }, function(err, data) {
      if (err) {
        self.apos.utils.error(err);
        return;
      }
      return res.send({
        list: self.render(req, 'committable-list.html', data),
        pager: self.render(req, 'committable-pager.html', data) 
      });
    });
  });
