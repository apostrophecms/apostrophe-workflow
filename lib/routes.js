...
module.exports = function(self, options) {
  self.addRoutes = function() {
    ...
    self.route('post', 'force-export', function(req, res) {
      return self.forceExport(req, req.body.id, req.body.locales, {
        existing: (!!req.body.lead) || (!!req.body.existing)
      ...
      function run(req, reporting, callback) {
        ...
        return async.series({
          addRelated: function(callback) {
            ...
            function fetchBatch() {
              ...
              if (!batch.length) {
                ...
                // Process related docs before selected docs,
                // so that joins pencil out correctly
                tasks = relatedIds.map(function(id) {
                  return {
                    _id: id,
          ...
          sortIds: function(callback) {
            ...
            // Ensure pages are force-exported parents-first. Everything
            // else stays in the original "related first, then selected" order
            return self.apos.docs.db.find({
              _id: { $in: _.pluck(tasks, '_id') },
              slug: /^\//
            }).project({ _id: 1 }).sort({ level: 1, rank: 1 }).toArray(function(err, docs) {
              if (err) {
                return callback(err);
              }
              const isPage = Object.fromEntries(docs.map(doc => [ doc._id, true ]));
              const newTasks = [];
              const pageTasks = [];
              ...
              const ordinals = Object.fromEntries(docs.map((doc, i) => [ doc._id, i ]));
              pageTasks.sort(function(a, b) {
                if (ordinals[a._id] < ordinals[b._id]) {
                  return -1;
                } else if (ordinals[a._id] > ordinals[b._id]) {
                  return 1;
                } else {
                  return 0;
                }
              });
              tasks = [ ...newTasks, ...pageTasks ];
              return callback(null);
            });
          },
    ...
    self.route('post', 'count-related-by-type', function(req, res) {
      var counts = {};
      // findRelated is actually quite fast, but fetching 1000 docs
      // one at a time is not. Fetch batches of 100 docs.
      // Was also tested at batch size 1 to make sure
      ...
      function fetchBatch() {
        ...
        if (!batch.length) {
          ...
          _.each(counts, function(count, type) {
            ...
            var pageType = _.find(self.apos.pages.options.types, { name: type });
            if (pageType) {
              labels[type] = pageType.label + ' (pages)';
            } else {
              labels[type] = manager.options.pluralLabel || manager.options.label || type;
            }
          });
          var pieceTypes = [];
          var pageTypes = [];
          // In configuration order, but we move page types to the end,
          // so they are less likely to be accidentally force exported
          _.each(self.apos.docs.managers, function(manager, name) {
            if (counts[name]) {
              if (self.apos.instanceOf(manager, 'apostrophe-custom-pages')) {
                pageTypes.push(name);
              } else {
                pieceTypes.push(name);
              }
            }
          });
          var types = pieceTypes.concat(pageTypes);
          return res.send({
            status: 'ok',
            html: self.render(req, 'relatedByType', {
              counts: counts,
              labels: labels,
              types: types
            })
        ...
        return self.findDocs(req, { _id: { $in: batch } }).areas(true).joins(true).toArray(function(err, docs) {
          ...
          return self.getRelated(docs, function(err, related) {
            ...
            _.each(related, function(doc) {
              if (!_.contains(req.body.ids, doc._id)) {
                counts[doc.type] = counts[doc.type] || 0;
                counts[doc.type]++;
              }
            });
            return fetchBatch();
    ...
    self.route('post', 'submit', function(req, res) {
      ...
      return async.eachSeries(ids, function(id, callback) {
        ...
        function submit(callback) {
          submission = self.getWorkflowSubmittedProperty(req, {
            type: 'submit'
          });
          ...
          return self.apos.docs.db.update({
            ...
            $set: {
              workflowSubmitted: submission
            }
          }, callback);
        }
        function emitAfterSubmit (callback) {
          submission._docId = id;

          return self.emit('afterSubmit', req, submission).then(function () {
            callback(null);
            return null;
          }).catch(function (e) {
            return callback(e);
    ...
    // via joins or widgets. However if `onlyIfNewIn` is also
    // present, related documents are returned only if they
    // do not already exist in one of the locales in that array.
    //
    ...
    self.route('post', 'editable', function(req, res) {
      ...
      var ids = self.apos.launder.ids(req.body.ids);
      var onlyIfNewIn = req.body.onlyIfNewIn ? self.apos.launder.strings(req.body.onlyIfNewIn) : null;
      return self.getEditable(req, ids, { related: req.body.related, onlyIfNewIn: onlyIfNewIn }, function(err, modified, unmodified, committable) {
        if (err) {
          self.apos.utils.error(err);
          return res.send({ status: 'error' });
        }
    ...
    self.route('post', 'force-export-modal', function(req, res) {
      ...
      return self.findDocs(req, { _id: id }).toObject(function(err, doc) {
        ...
        var nestedLocales = self.getEditableNestedLocales(req, doc);
        return res.send(self.render(req, 'force-export-modal.html', {
          doc: doc,
          nestedLocales: nestedLocales,
          lead: lead,
          offerRelatedExisting: (self.options.forceExportExistingRelatedDocuments !== false)
        }));
      });
    });

    self.route('post', 'force-export-related-modal', function(req, res) {
      if (!req.user) {
        // Confusion to the enemy
        return res.status(404).send('not found');
      }
      ...
      return self.findDocs(req, { _id: id }).toObject(function(err, doc) {
        ...
        var nestedLocales = self.getEditableNestedLocales(req, doc);
        return res.send(self.render(req, 'force-export-related-modal.html', {
          doc: doc,
          nestedLocales: nestedLocales,
          lead: lead,
          offerRelatedExisting: (self.options.forceExportExistingRelatedDocuments !== false)
    ...
    self.route('post', 'batch-force-export-modal', function(req, res) {
      ...
      return self.findDocs(req, { _id: ids[0] }).toObject(function(err, doc) {
        ...
        var nestedLocales = self.getEditableNestedLocales(req, doc);
        return res.send(self.render(req, 'batch-force-export-modal.html', {
          ids: ids,
          locale: self.liveify(doc.workflowLocale),
          nestedLocales: nestedLocales,
          offerRelatedExisting: (self.options.forceExportExistingRelatedDocuments !== false)
        }));
      });
    ...
    self.route('get', 'link-to-locale', function(req, res) {
      ...
      return self.apos.docs.find(req, criteria).published(null).toObject(function(err, doc) {
        ...
        if (doc && doc._url) {
          // Use rawRedirect because the _url property will already
          // have any sitewide prefix
          return res.rawRedirect(self.apos.urls.build(doc._url, { workflowLocale: locale }));
        } else {
          // If no matching document was found, we intentionally redirect
          // the visitor to the slug they requested, where they will see a
          // a familiar 404 error. We do not use rawRedirect here because it
          // is just a slug and they do not carry the prefix in the db
          return res.redirect(self.apos.urls.build(slug, { workflowLocale: locale }));
        }
      });
    });
