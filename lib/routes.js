...
module.exports = function(self, options) {
  ...
  self.route('post', 'locale-unavailable-modal', function(req, res) {
    ...
    return self.getAvailability(req, workflowGuid, locale, function(err, status) {
      if (err || (status === 'notfound')) {
        return res.status(500).send('error');
      }
      return res.send(self.render('locale-unavailable-modal', { status: status, mode: req.session.workflowMode, workflowGuid: workflowGuid, locale: locale }));
    });
  });
  ...
  self.route('post', 'activate', function(req, res) {
    ...
    function getAvailability(callback) {
      return self.getAvailability(req, workflowGuid, locale, function(err, _status) {
        if (err) {
          return callback(err);
        }
        status = _status;
        if ((status === 'notfound') || (status === 'available')) {
