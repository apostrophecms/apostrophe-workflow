...
describe('Workflow Add Missing Locales Inheritance', function() {
  ...
  it('Add draft of document for testing', () => {
    apos.docs.db.find({ title: 'test', workflowLocale: 'us-en' }).toArray().then(function(docs) {
      console.log('DD', docs[0]);
      const draft = Object.assign(docs[0], {
        "body" : {
           "items" : [ ],
           "type" : "area"
        }        
      ...
      console.log("DD", draft);
		  apos.docs.db.update({_id: draft._d}, draft);	
    });
  });
});
